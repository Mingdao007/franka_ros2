...
| `franka_gazebo/franka_gazebo_bringup/launch/gazebo_hybrid_circle_force.launch.py` | Launch: robot description (ball-tip), Gazebo, RViz (optional), controller spawning. |
| `franka_gazebo/franka_gazebo_bringup/config/franka_gazebo_controllers.yaml` | All tunable parameters: gains, descent, circle, ink trail, force PID. |
| `franka_gazebo/franka_gazebo_bringup/config/hybrid_circle_force.rviz` | RViz config: world frame, InkTrail (blue) + InkDesired (red) marker displays, top-down view. |
| `franka_gazebo/franka_gazebo_bringup/config/gz_gui.config` | Gazebo GUI camera layout. |
...
| `generate_hybrid_experiment_report.py` | Computes gates and metrics, outputs markdown report + plots. |
